...
void vDXLRTask( void *pvParameters )
{
    ...
    const portTickType xFrequency = 50;  //this mean each 10 mili secound it shoud be run (100Hz)
    ...
    for(;;)
    {
      ...
      if((DXLRTaskCNT)==1)  //ping motors each 100 ms for get the model number of dynamixels
      {        
        for(i=0;i<=NUM_OF_DXL-1;i++)
        {
          vTaskSuspendAll();   //Suspend all other tasks
          PRESENT_MODEL_NUMBER[i]=DXL.readWord(i+1,P_MODEL_NUMBER_L); //read the model number of conected dynamixel and recognize it for the send and recive
          xTaskResumeAll();   //Resume all other tasks 
        }               
      }
      
      //this loop is runs about the number of dynamixel in bus and update global arrays of reading value from motors
      for(i=0; i<=NUM_OF_DXL-1; i++ )
      {
        if(PRESENT_MODEL_NUMBER[i]!=65535) //if we found the motor in the bus with this id
        { 
          DXL_Exist[i]=1; //update the dxl exist array for some of dynamixel that in the bus
