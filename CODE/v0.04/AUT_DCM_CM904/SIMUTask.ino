...
float betaDef=0.08;                      // 2 * proportional gain
...
void vSIMUTask( void *pvParameters )
{
  ...
  { 
    ...
    float Hz=(float)SIMUTaskHz; //Get current task frequency
    ...
    getRollPitchYaw(0.1);   //To calculate angle from quaternian. void getRollPitchYaw(double low_pass_present) 
...
*/
void getRollPitchYaw(double low_pass_present) 
{
  roll  =((low_pass_present) * P_roll)  + ((1-low_pass_present)*  (atan2(2 * (q0 * q1 + q2 * q3) ,1 - 2 * (q1 * q1 + q2 * q2))*RAD2DEG));
  pitch =((low_pass_present) * P_pitch) + ((1-low_pass_present)*  (asin (2 * (q0 * q2 - q3 * q1))*RAD2DEG));
  yaw   =((low_pass_present) * P_yaw)   + ((1-low_pass_present)*  (atan2(2 * (q0 * q3 + q1 * q2) , 1 - 2* (q2 * q2 + q3 * q3))*RAD2DEG));
